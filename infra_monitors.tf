...
resource "datadog_monitor" "container_count" {
  ...
  query   = "max(last_10m):max:datadog.estimated_usage.containers{*}  > ${var.commited_hosts * 5}"
  tags    = ["service:datadog_usage"]
}
...
resource "datadog_monitor" "custom_metrics" {
  ...
  query   = "max(last_10m):max:datadog.estimated_usage.metrics.custom{*}  > ${var.commited_hosts * 100}"
  tags    = ["service:datadog_usage"]
}
...
resource "datadog_monitor" "custom_events" {
  ...
  query   = "max(last_10m):max:datadog.estimated_usage.events.custom_events{*}  > ${var.commited_hosts * 500}"
  tags    = ["service:datadog_usage"]
}
