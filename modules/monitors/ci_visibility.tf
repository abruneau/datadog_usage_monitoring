resource "datadog_monitor" "pipeline_committers" {
  ...
  query   = "max(last_10m):sum:datadog.estimated_usage.ci_visibility.pipeline.committers{*}  > ${var.commited_ci_visibility_pipeline_committers}"
...
resource "datadog_monitor" "test_committers" {
  ...
  query   = "max(last_10m):sum:datadog.estimated_usage.ci_visibility.test.committers{*}  > ${var.commited_ci_visibility_test_committers}"
  tags    = var.monitors_tags
}